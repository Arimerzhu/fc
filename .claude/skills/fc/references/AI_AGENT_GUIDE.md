...
fc part boolean cut Box_001 Cyl_001 --name Result_001 --json
...
| 用户意图 | 命令 |
|----------|------|
| 创建盒子 | `fc --json part add box --name Box --param Length=20 --param Width=15 --param Height=10` |
| 创建圆柱 | `fc --json part add cylinder --name Cyl --param Radius=5 --param Height=20` |
| 布尔并集 | `fc --json part boolean fuse Box Cyl --name Fused` |
| 布尔切割 | `fc --json part boolean cut Box Cyl --name Cut` |
| 布尔交集 | `fc --json part boolean common Box Cyl --name Common` |
| 导出 STEP | `fc --json export step --output model.step --overwrite` |
| 导出 STL | `fc --json export stl --output model.stl --tolerance 0.05 --overwrite` |
| 保存项目 | `fc --json document save --output project.FCStd --overwrite` |
| 撤销 | `fc --json session undo` |
| 创建快照 | `fc --json session snapshot v1 --description "初始设计"` |
| 创建草图 | `fc --json sketch new --name MySketch --plane XY` |
| 添加圆 | `fc --json sketch add-circle MySketch --center 0,0 --radius 10` |
| 拉伸 | `fc --json body pad Body001 Sketch001 --length 20` |
| 倒角 | `fc --json part fillet-3d Box --radius 2` |
| 倒直角 | `fc --json part chamfer-3d Box --size 1.5` |
| 镜像 | `fc --json part mirror Box --plane XY --name BoxMirrored` |
| 缩放 | `fc --json part scale Box 1.5` |
| 创建孔 | `fc --json part hole Box --diameter 8 --depth 10` |
| 创建工程图 | `fc --json techdraw page --name Page1 --format A3` |
| 添加视图 | `fc --json techdraw view --page Page1 --source Box --direction 0,0,1 --scale 1.0` |
| 创建电子表格 | `fc --json spreadsheet create --name Params` |
| 设置参数 | `fc --json spreadsheet set --sheet Params --cell A1 --value 100` |
| 分配材料 | `fc --json material assign --object Box --material Steel` |
| 导入 STEP | `fc --json import step model.step` |
| 执行 Python | `fc --json execute code "print(FreeCAD.ActiveDocument.Name)"` |
